...
import { waitForTask as waitForApplicationTask } from "../application/tasks/wait-for-task.js";
import { isAuthenticated } from "../config.js";
import { createGateways } from "../application/composition/create-gateways.js";
...
async function resolveProject(projectsGateway, reference) {
  if (reference === undefined || reference === null || reference === "") return null;

  const projects = asItems(await projectsGateway.list());
...
export async function resolveApp(api, reference) {
  const apps = asItems(await api.list());
  return resolveAppReference(apps, reference);
}

export function createFabrokuToolHandlers(options = {}) {
  const api = options.api || new FabrokuAPI();
  const gateways = options.gateways || createGateways(api);
  ...
  const wait = (appId, taskId) => waitForApplicationTask({
    getStatus: () => gateways.apps.status(appId),
    taskId,
    sleep,
    pollIntervalMs,
    maxPolls,
  ...
  return {
    listProjects: run(async () => {
      const projects = asItems(await gateways.projects.list()).map(sanitizeProject);
      return successResult({ projects, count: projects.length });
    }),

    listApps: run(async ({ project } = {}) => {
      const selectedProject = await resolveProject(gateways.projects, project);
      let apps = asItems(await gateways.apps.list());
      if (selectedProject) {
    ...
    getApp: run(async ({ app }) => {
      const selectedApp = await resolveApp(gateways.apps, app);
      const details = await gateways.apps.get(selectedApp.id);
      return successResult({ app: sanitizeApp(details) });
    }),

    listServices: run(async ({ app, project } = {}) => {
      const selectedApp = app ? await resolveApp(gateways.apps, app) : null;
      const selectedProject = project ? await resolveProject(gateways.projects, project) : null;
      const response = await gateways.services.list({
        app: selectedApp?.id,
    ...
    getStatus: run(async ({ app }) => {
      const selectedApp = await resolveApp(gateways.apps, app);
      const status = await gateways.apps.status(selectedApp.id);
      return successResult({ app: sanitizeApp(selectedApp), task: status });
    }),

    getRuntimeLogs: run(async ({ app, lines = 100 }) => {
      const selectedApp = await resolveApp(gateways.apps, app);
      const response = await gateways.apps.runtimeLogs(selectedApp.id, lines);
    ...
    redeploy: run(async ({ app, confirmed_committed_and_pushed, wait: shouldWait = true }) => {
      ...
      const selectedApp = await resolveApp(gateways.apps, app);
      const started = await gateways.apps.redeploy(selectedApp.id);
      if (!shouldWait) {
        return successResult({
          app: { id: selectedApp.id, name: selectedApp.name },
    ...
    runMigrate: run(async ({ app, manage_path = "manage.py", noinput = true, wait: shouldWait = true }) => {
      const selectedApp = await resolveApp(gateways.apps, app);
      const started = await gateways.apps.runMigrate(selectedApp.id, { manage_path, noinput });
      if (!shouldWait) {
        return successResult({
          app: { id: selectedApp.id, name: selectedApp.name },
          task_id: started.task_id,
