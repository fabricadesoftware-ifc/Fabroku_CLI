...
import { createTerminalInputPump as createInputPump } from "../infrastructure/terminal/input-pump.js";
...
async function consumePostgresSession(api, appId, sessionId) {
  ...
  const inputPump = createInputPump({
    sendInput: (data) => sessionGateway.input(appId, sessionId, { data }),
  });
  ...
  try {
    while (!finished) {
      ...
      try {
        await sessionGateway.terminalEvents(appId, sessionId, {
...
async function consumePostgresSessionWebSocket(api, session) {
  ...
  const inputPump = createInputPump({
    ...
    onError: (error) => process.stderr.write(chalk.red(`\nErro ao enviar input: ${error.message}\n`)),
  });
  ...
  const cancel = async () => {
    ...
    try {
      if (socket && socket.readyState === WebSocket.OPEN) {
        ...
        await createInteractiveSessionGateway(api).cancel(session.app_id, session.session_id);
      }
    } catch {
