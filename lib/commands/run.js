...
import { createInteractiveSessionGateway } from "../application/gateways/interactive-session-gateway.js";
import { normalizeSessionEvent } from "../domain/sessions/session-event.js";
import { isAuthenticated } from "../config.js";
import { readSecretInput } from "../infrastructure/terminal/secret-input.js";
...
async function pollRunStatus(api, appId, taskId) {
  let lastStatus = "";
  let result;
  try {
    result = await waitForTask({
    getStatus: () => createAppGateway(api).status(appId),
      taskId,
      sleep,
      pollIntervalMs: 3000,
      maxPolls: 240,
    ...
    result = { success: false, status: { status: error.message } };
  }
...
function promptHidden(promptText) {
  ...
  return readSecretInput({ prompt: promptText });
}
...
async function cancelInteractiveSession(api, appId, sessionId, controller) {
  ...
  try {
    await createInteractiveSessionGateway(api).cancel(appId, sessionId);
  } catch {
...
async function consumeInteractiveSession(api, appId, sessionId) {
  const sessionGateway = createInteractiveSessionGateway(api);
  ...
  try {
    while (!finished) {
      ...
      try {
        await sessionGateway.events(appId, sessionId, {
        ...
        if (pendingAnswer && !cancellationRequested) {
          await sleep(100);
          try {
            await sessionGateway.answer(appId, sessionId, pendingAnswer);
          } catch (error) {
            handleApiError(error);
          } finally {
            pendingAnswer = null;
  ...
  const onSigint = async () => {
    ...
    try {
      if (socket && socket.readyState === WebSocket.OPEN) {
        ...
        await sessionGateway.cancel(session.app_id, session.session_id);
      }
    } catch {
...
export async function runLoaddata(fixturePath, options) {
  ...
  let task;
  try {
    task = await appGateway.runLoaddata(app.id, {
  ...
  const result = await pollRunStatus(api, app.id, task.task_id);
  if (!result.success) {
    console.log(chalk.red(`loaddata falhou: ${result.status?.status || "erro desconhecido"}`));
    process.exit(1);
  }
...
export async function runMigrate(options) {
  ...
  let task;
  try {
    task = await appGateway.runMigrate(app.id, {
  ...
  const result = await pollRunStatus(api, app.id, task.task_id);
  if (!result.success) {
    console.log(chalk.red(`migrate falhou: ${result.status?.status || "erro desconhecido"}`));
    process.exit(1);
  }
...
export async function runDumpdata(options, dumpArgs = []) {
  ...
  let task;
  try {
    task = await appGateway.runDumpdata(app.id, {
      manage_path: options.manage || "manage.py",
      dump_args: dumpArgs,
  ...
  const result = await pollRunStatus(api, app.id, task.task_id);
  if (!result.success) {
    console.log(chalk.red(`dumpdata falhou: ${result.status?.status || "erro desconhecido"}`));
    process.exit(1);
  }
...
export async function runCreatesuperuser(options) {
  ...
  let session;
  try {
    session = await createInteractiveSessionGateway(api).create(app.id, {
